...
public class PaymentServiceImpl implements PaymentService{
    ...
    public User newUser(String userName, String email, Double creditLimit) {
        User newUser = User.builder().userName(userName).email(email).
                creditLimit(creditLimit).dues(0D).build();
    ...
    public Merchant addMerchant(String merchantName, String email, Double discountPercent) {
        Merchant newMerchant = Merchant.builder().merchantName(merchantName).
                email(email).discountPercent(discountPercent).totalAmount(0D).build();
    ...
    public String newTransaction(String userName, String merchantName, Double transactionAmount) {
        ...
        merchant.setTotalAmount(
                merchant.getTotalAmount()  + ((100 - discountRate) * transactionAmount) / 100);
    ...
    public List<String> reportUsersAtCreditLimit() {
        return userList.stream().filter(
                x -> Math.abs(x.getDues()) >= x.getCreditLimit()).map(
                        x-> x.getUserName()).collect(Collectors.toList());
    }
